namespace roundhouse.databases.oracle.orm
{
    ...
    public class ScriptsRunErrorMapping : ClassMap<ScriptsRunError>
    {
        public ScriptsRunErrorMapping()
        {
            ...
            Map(x => x.script_name);
            Map(x => x.text_of_script).CustomType("AnsiString").CustomSqlType("Clob");
            Map(x => x.erroneous_part_of_script).CustomType("AnsiString").CustomSqlType("Clob");
            Map(x => x.error_message).CustomType("AnsiString").CustomSqlType("Clob");
